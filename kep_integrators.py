...
# a dictionary which puts the above gradient functions to string repr
flow_dic = {'no proj':mod_flow_o2_no_proj_unflattened, 
            'energy':mod_flow_o2_energy_proj, 
            'ang mom':mod_flow_o2_ang_proj, 
            'both':mod_flow_o2_energy_and_ang_proj} 

d2_dic = {"exp euler":d2_exp_euler,
          "stromer verlet":d2_stromer_verlet}
...
def fourth_order_kutta_modified_flow(y , d2_name="exp euler" ,
                                     h=0.01 , flow_name='energy'):
    d2 = d2_dic[d2_name]
    flow = flow_dic[flow_name] 
    k1 = flow(y,d2,h)
    k2 = flow(y + 0.5*H*k1,d2,h) 
    k3 = flow(y + 0.5*H*k2,d2,h)
    k4 = flow(y + H*k3,d2,h) 
    y_next = y + H/6*(k1 + 2*k2 + 2*k3 + k4)
    return y_next
